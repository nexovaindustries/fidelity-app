// POST /api/wallet/notify-update
// Avisa (push APNs silencioso) a todos los dispositivos Apple Wallet con un pase
// instalado de este comercio que hay datos nuevos (logo/icono/color/etc.) para que
// Apple vuelva a pedir el .pkpass actualizado. No cambia texto visible del pase.
// Body: { comercio_id: "uuid" }
// Seguridad: requiere sesión Supabase del dueño del comercio.
...
export async function onRequest(context) {
  ...
  const { comercio_id } = body;
  if (!comercio_id) {
    return new Response(JSON.stringify({ error: 'comercio_id es requerido' }), { status: 400, headers: corsHeaders });
  }
  ...
  if (!tarjetas?.length) {
    return new Response(JSON.stringify({ message: 'No hay tarjetas activas', push_enviados: 0 }), { headers: corsHeaders });
  }
  ...
  if (!registrations?.length) {
    return new Response(JSON.stringify({
      message: 'Datos actualizados. Ningún cliente tiene Apple Wallet registrado aún.',
...
-----END PRIVATE KEY-----`;
  ...
  return new Response(JSON.stringify({
    message: 'Notificación enviada a dispositivos Apple Wallet.',
    tarjetas_actualizadas: ids.length,
